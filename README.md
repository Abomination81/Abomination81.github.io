...
| Hero variants | `assets/hero.js` swaps the portrait for a video when the URL has `?hero=kling`, `?hero=seedance` or `?hero=hailuo`. Files live in `assets/hero/`. The unlisted `/ab/` page compares them. |
